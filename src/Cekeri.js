      ...
      {checkers.map((ceker, l) => (
        ...
        <Card border="dark">
          ...
          <Card.Body style={{margin: '5px', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <Button onClick={() => handleProductClick(ceker)} size="sm" variant='outline-dark'>Pogledaj</Button>
          
            <Button onClick={() => handleAddToCart(ceker)} style={{ marginTop: '5px' }} size="sm" variant='outline-dark' disabled={addedToCart.includes(ceker)}>{addedToCart.includes(ceker) ? 'Dodano u korpu' : 'Dodaj u Korpu'}</Button>
        </Card.Body>
